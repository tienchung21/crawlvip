#!/usr/bin/env python3
"""
Merge duan_meeyland -> duan by project name.

Khong sua bang duan. Chi ghi bang moi:
  duan_meeyland_duan_merge
"""
...
def ensure_merge_table(db: Database) -> None:
    ...
    try:
        cur.execute(
            """
            CREATE TABLE IF NOT EXISTS duan_meeyland_duan_merge (
                meeyland_project_pk INT NOT NULL,
                meeyland_project_id VARCHAR(100) NOT NULL,
                meeyland_project_name VARCHAR(255) NOT NULL,
                ...
                PRIMARY KEY (meeyland_project_pk),
                UNIQUE KEY uq_meeyland_project_id (meeyland_project_id),
...
def truncate_merge_table(db: Database, dry_run: bool) -> None:
    ...
    cur = conn.cursor()
    try:
        cur.execute("TRUNCATE TABLE duan_meeyland_duan_merge")
        conn.commit()
    finally:
        cur.close()
        conn.close()
...
def fetch_meeyland_projects(db: Database, limit: int = 0) -> List[Tuple[int, str, str]]:
    conn = db.get_connection()
    cur = conn.cursor()
    try:
        sql = """
        ...
        if limit > 0:
            ...
            cur.execute(sql)
        return [(int(r["id"]), r.get("project_id") or "", r.get("name") or "") for r in cur.fetchall()]
    finally:
        cur.close()
        conn.close()
...
def insert_mappings(
    ...
    try:
        cur.executemany(
            """
            INSERT INTO duan_meeyland_duan_merge
                (meeyland_project_pk, meeyland_project_id, meeyland_project_name, duan_id, duan_ten, duan_tinh_moi, match_type, score)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
            ON DUPLICATE KEY UPDATE
                meeyland_project_id=VALUES(meeyland_project_id),
                meeyland_project_name=VALUES(meeyland_project_name),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Merge duan_meeyland -> duan by project name")
    ...
    t0 = time.time()
    duans = fetch_duan(db)
    src_rows = fetch_meeyland_projects(db, limit=args.limit)
    ...
    for src_pk, src_id, src_name in src_rows:
        d = exact_by_ten.get(src_name)
        if d:
            batch.append((src_pk, src_id, src_name, d.duan_id, d.duan_ten, d.duan_tinh_moi, "exact", 1.0))
            mapped += 1
            mapped_exact += 1
            if len(batch) >= 1000:
        ...
        if d:
            batch.append((src_pk, src_id, src_name, d.duan_id, d.duan_ten, d.duan_tinh_moi, "norm_exact", 1.0))
            mapped += 1
            mapped_norm += 1
            if len(batch) >= 1000:
                insert_mappings(db, batch, args.dry_run)
        ...
        if c:
            ...
            if d:
                batch.append((src_pk, src_id, src_name, d.duan_id, d.duan_ten, d.duan_tinh_moi, "core_exact", 1.0))
                mapped += 1
                mapped_norm += 1
                if len(batch) >= 1000:
                    insert_mappings(db, batch, args.dry_run)
        ...
        if args.no_fuzzy:
            unmatched.append((src_pk, src_id, src_name))
        else:
            toks = tokenize(n)
            ...
            if best:
                ...
                if score >= args.min_score:
                    batch.append((src_pk, src_id, src_name, bd.duan_id, bd.duan_ten, bd.duan_tinh_moi, "fuzzy", float(score)))
                    mapped += 1
                    mapped_fuzzy += 1
                else:
                    unmatched.append((src_pk, src_id, src_name))
