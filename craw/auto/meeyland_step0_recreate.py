...
def main():
    ...
    print("=== meeyland_step0_recreate.py ===")
    start = time.time()
    ...
    if not rows:
        print("No raw meeyland records left to migrate.")
        return
    ...
    sql = f"""
        ...
        SELECT 
            ...
            IF(sc.ngaydang IS NULL OR sc.ngaydang = '', UNIX_TIMESTAMP(sc.created_at), UNIX_TIMESTAMP(STR_TO_DATE(sc.ngaydang, '%%Y-%%m-%%dT%%H:%%i:%%s.%%fZ'))), 
            UNIX_TIMESTAMP(sc.created_at),
            dm.duan_id,
            0, 'meeyland_step0_recreate.py'
        FROM scraped_details_flat sc
        LEFT JOIN duan_meeyland_duan_merge dm ON sc.thuocduan COLLATE utf8mb4_general_ci = dm.meeyland_project_id COLLATE utf8mb4_general_ci
    ...
    print(f"-> Migrated {inserted} meeyland records into data_clean_v1 out of {len(rows)} raw fetched, in {time.time()-start:.2f}s.")
    cursor.close()
    conn.close()
