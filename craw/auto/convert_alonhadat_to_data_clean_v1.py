...
SCRIPT_NAME = "convert_alonhadat_to_data_clean_v1.py"
...
SALE_RAW_KEEP_SQL = ", ".join(f"'{x}'" for x in sorted(SALE_RAW_KEEP))
...
def make_ad_id(source_post_id: Any) -> str:
    return f"alonhadat_{source_post_id}"
...
def median_group_for(row: Dict[str, Any]) -> Optional[int]:
    ...
    if trade_type != "s":
        return None
    if loaibds in ("Nhà mặt tiền", "Nhà trong hẻm", "Biệt thự, nhà liền kề"):
        return 1
    if loaibds in ("Căn hộ chung cư", "Shop, kiot, quán"):
        return 2
    if loaibds in (
        "Mặt bằng",
        "Đất thổ cư, đất ở",
...
def price_land_for(row: Dict[str, Any]) -> Optional[int]:
    ...
    if loaibds in (
        ...
        return price_vnd
    if loaibds in ("Kho, xưởng", "Trang trại"):
        return int(price_vnd * 0.85)
    if loaibds in ("Nhà mặt tiền", "Nhà trong hẻm"):
        return int(price_vnd * 0.75)
    if loaibds == "Biệt thự, nhà liền kề":
        return int(price_vnd * 0.65)
    return None
...
def land_price_status_for(row: Dict[str, Any]) -> str:
    trade_type = (row.get("trade_type") or "").strip()
    loaibds = (row.get("loaibds") or "").strip()
    if trade_type == "s" and loaibds in (
        "Đất thổ cư, đất ở",
        "Đất nền, liền kề, đất dự án",
        "Đất nông, lâm nghiệp",
        "Kho, xưởng",
...
def build_payload(row: Dict[str, Any]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    if trade_type == "s" and loaibds not in SALE_RAW_KEEP:
        return None, "skip_type"
    if trade_type == "u" and not loaibds:
        return None, "skip_type"
    ...
    median_group = median_group_for(row)
    if median_group is None and land_price_status_for(row) is None:
        ...
        sql = f"""
            ...
            WHERE sdf.domain = %s
              ...
              AND (
                 ...
                 OR (sdf.trade_type = 's' AND sdf.loaibds IN ({SALE_RAW_KEEP_SQL}))
              )
...
def main():
    parser = argparse.ArgumentParser(description="Convert Alonhadat data_full + detail_flat -> data_clean_v1")
