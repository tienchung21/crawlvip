...
def parse_price_to_vnd(price_str):
    ...
    if 'tỷ' in p:
        ...
        return None  # Không nhận diện được đơn vị

    # Lấy số (hỗ trợ cả dấu chấm và dấu phẩy)
...
def main():
    ...
    script_name = "nhatot_step2_normalize_price.py"
    ...
    while True:
        # Lấy batch dữ liệu cần xử lý (đã xong Step 1)
        sql_get = f"""
            SELECT id, src_price 
            FROM data_clean_v1 
    ...
    # Finalize: Nâng process_status lên 2 cho tất cả dòng đã xong Step 1
