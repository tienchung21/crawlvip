...
def main():
    ...
    print("=== meeyland_step3_normalize_size.py ===")

    start = time.time()
    script_name = 'meeyland_step3_normalize_size.py'
        ...
        SET
            std_area = CASE
                WHEN REPLACE(TRIM(src_size), ',', '.') REGEXP '^[0-9]+(\\\.[0-9]+)?$'
                     AND CAST(REPLACE(TRIM(src_size), ',', '.') AS DECIMAL(12,4)) > 0
                THEN CAST(REPLACE(TRIM(src_size), ',', '.') AS DECIMAL(10,2))
                ELSE NULL
            END,
            unit = CASE
                WHEN REPLACE(TRIM(src_size), ',', '.') REGEXP '^[0-9]+(\\\.[0-9]+)?$'
            ...
            price_m2 = CASE
                WHEN price_vnd IS NOT NULL
                     ...
                     AND CAST(REPLACE(TRIM(src_size), ',', '.') AS DECIMAL(12,4)) > 0
                THEN price_vnd / CAST(REPLACE(TRIM(src_size), ',', '.') AS DECIMAL(12,4))
                ELSE NULL
            END,
            process_status = CASE
                WHEN REPLACE(TRIM(src_size), ',', '.') REGEXP '^[0-9]+(\\\.[0-9]+)?$'
    ...
    print(f"-> Normalized size, price_m2 and set status (3 or -3) for {updated} meeyland records in {time.time()-start:.2f}s.")
    cursor.close()
    conn.close()
