...
<div class="container">
    ...
    <div style="display:inline-flex; align-items:center; margin-left:8px;">
        <select id="batch-month-select" style="padding:8px; background:#1b1f27; color:#fff; border:1px solid #2c3240; border-radius:6px; margin-right:8px;">
            <option value="">-- Tat ca thang --</option>
            @foreach ($months as $m)
                <option value="{{ $m }}">{{ $m }}</option>
            @endforeach
        </select>
        <button class="btn" type="button" id="convert-batch">Convert stats (batch)</button>
    </div>
...
<script>
    (function () {
        ...
        var monthSelect = document.getElementById("batch-month-select");
        ...
        if (btn && progress) {
            ...
            btn.addEventListener("click", async function () {
                ...
                progress.textContent = "Dang chay... (Thang: " + (selectedMonth || "Tat ca") + ")";
                ...
                while (true) {
                    ...
                    if (selectedMonth) {
                        body.append("month", selectedMonth);
                    }
